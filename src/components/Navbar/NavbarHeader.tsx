...
export const NavbarHeader: React.FC<NavbarHeaderProps> = ({ isOpen }) => {
  ...
  return (
    <div className={`flex items-center flex-col gap-4 py-2 ${isOpen}`}>
      ...
      <div>
        <div className="rounded-full w-[35px] h-[35px] lg:w-[100px] lg:h-[100px] bg-slate-300 bg-[url(/images/my.png)] bg-cover bg-center" />
      </div>
      <div className="relative">
        <img src="/images/verified.png" alt="verified" className="absolute -left-6 top-1" />
        <h1 className="text-lg font-serif font-bold">Muhammad Nur Maulana</h1>
        <p className="font-serif hidden lg:block">@maulana</p>
        <button className={`w-20 p-1 bg-[url(/images/retro.jpg)] bg-cover rounded-2xl mt-4 bg-bottom ${change ? "bg-[url(/images/retro.jpg)]" : "bg-[url(/images/car.jpg)]"}`} onClick={changeToggleTheme}>
          <div className={`w-4 h-4 rounded-full bg-white bg-cover bg-center ${change ? "animate-toRight" : "animate-toLeft"}`}>
